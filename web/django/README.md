...
actually, manage.py is exactly trying to help us simplify this.  
...
The reason that we need to use __ is that we want to specify the relationship == something, not a variable = something 
...
order_by() can be place anywhere (doesn't matter before or or after filter ),
because ultimately will be put at nearly last in the sql command generated by django.  
...
in the url() function, why do we need " the name='results' "?  
...
# in this particular case, the order of display will be pub_date and then question_text (default behavior is the other way)
