...
If you were to now export another overlay, such as our favorite UART1 overlay, you would see a new option listed as number 6.
Let's try that by exporting the UART1 dtbo file:

```
    root@beaglebone:/sys/devices/bone_capemgr.8# echo BB-UART1 > slots
```

We are taking the output of echo, "BB-UART1", and writing it to the slots file to enable the drivers and device for UART1 using the overlay.
Now, let's check that the overlay loaded properly:
...
However, apparently, not all pins will be shown here.
For example, after exporting the analog pins device tree overlay, the pins are not shown here.

Also, when using LCD, exporting the analog pins device tree doesn't complain, but the pins cannot be used.
There could be some conflicts between the hidden LCD pins and analog pins.
...
Using this package directly, my test can get 833Hz on one channel only. 
There should be some ways to speed up the sampling frequency, because the internal ADC is at least 1MHz.

When the LCD screen is used, activating the device tree is okay.
...
```
cd /sys/class/leds/beaglebone\:green\:usrX/  [X=0/1/2/3]
cat none/heartbeat/timer > trigger
```
...
References:  
https://kwantam.github.io/BBKNotes1.html (enable g_serial for ttyACM0)  
...
Control the pin at /sys/class/gpio/ folder. Examples:

```
echo 60 > /sys/class/gpio/export # Enable a gpio (e.g. gpio60); where 60 is the gpio number, not pin number: 

echo out > /sys/class/gpio/gpio60/direction # set direction as output

echo 1/0 > /sys/class/gpio/gpio60/value # set value as 1/0: 
```
...
To get the correct gpio number (which is not pin number), just use a table to map the pin number to gpio number.
E.g. p9 pin 12 corresponds gpio_60.  
...
(Due to some unknown reasons, my attemp with this method fails.)
...
Copy the zImage and device tree to the /dev/mmcblk0p2 root/boot folder in the BBB emmc.
Make symbolic links to all files so that appear on /boot folder first layer. (symbolic links are needed for zImage, arm*.dtb files)
...
And running with archlinux, the power consumption is about 200mA x 5V = 1W from the my experiment.
...
Some notes from reading BBB SRM Rev C.1
------------------------------------------
1. Power button can be used to put BBB to sleep and then to wake it up (Tested with Archlinux. However, BBB failed to wake up with some stack problem)
